...
class UsbPrinterManager @Inject constructor(
    ...
    private val attachReceiver = object : BroadcastReceiver() {
        override fun onReceive(ctx: Context?, intent: Intent?) {
            ...
            if (usbManager?.hasPermission(device) != true) {
                requestPermission(device.deviceName) { /* system dialog handles grant */ }
            }
        }
    }
    ...
    fun startMonitoring() {
        ...
        requestPermissionForAttachedDevices()
    }

    fun requestPermissionForAttachedDevices() {
        listDevices()
            .filter { !it.hasPermission }
            .forEach { device ->
                requestPermission(device.deviceName) { /* optional callback */ }
            }
    }
