...
export function DragHandle({
  ...
  return (
    <button
      ...
      className={
        className ||
        'cursor-grab touch-none rounded p-1 text-[var(--text-muted,#64748b)] hover:bg-[var(--bg-muted,#eef0f3)] active:cursor-grabbing'
      }
      aria-label={label}
      {...attributes}
      {...listeners}
...
type SortableContainerProps<T extends { id: string }> = {
  ...
  children: ReactNode;
  disabled?: boolean;
  /** Wrap SortableContext children in a semantic element (e.g. tbody for tables). */
